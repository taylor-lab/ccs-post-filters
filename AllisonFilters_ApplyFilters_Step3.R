...
for(j in 1:B)
{
  print(j);
  #
  if(j %ni% c(19,63,87,88,105,113,133))
  {
  ...
  batchid = str_replace(maf_file_base_name,".muts.maf",""); print(batchid)
  ...
  mfile_name = gsub(",", "",toString(mapping_files[as.numeric(grep(batchid,mapping_files))]))
 ...
 fill$Ttag <- paste(sapply(strsplit(as.character(fill$Tumor_Sample_Barcode),"[.]"),function(x){x[1]}),fill$Chromosome,fill$Start_Position,fill$Reference_Allele,fill$Tumor_Seq_Allele1,sep=";")
  ...
  ##Just carry forward one column fo removefinal
  #maf_ = maf_ %>% mutate(FILTER_css = plyr::mapvalues(var_tag, df$var_tag, df$removefinal))
  #table(maf_$FILTER_css)
  #table(maf_$FILTER)
  
  ##Clean up and format the FILTER column
  #maf_ = maf_ %>% mutate(FILTER = ifelse(FILTER_css == "remove",str_c(FILTER,",","ccs_manual_filter"),FILTER))
  #table(maf_$FILTER)
  #maf_ = maf_ %>% mutate(FILTER = ifelse(FILTER == "PASS,ccs_manual_filter","ccs_manual_filter",FILTER)) %>% select(-c(FILTER_css))
  ...
  table(maf_$FILTER)
  dim(maf_)
  write.table(maf_,paste0("/ifs/res/taylorlab/chavans/roslin_2.4_deliveries/cs_deliveries/final/",batchid,".muts.ccs_tagged.maf"),quote=FALSE,row.names = FALSE,sep="\t",append=FALSE)
  write.table(maf_,paste0(batch_out_dir,"/",batchid,".muts.ccs_tagged.maf"),quote=FALSE,row.names = FALSE,sep="\t",append=FALSE)
