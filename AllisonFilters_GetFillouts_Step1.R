...
N_bam_files = list.files(bam_dir,pattern=".*N00.*.bam$",full.names=TRUE)
N_bam_files
...
for(j in 1:B)
  ...
  maf_file_base_name = str_match(amaf_files[j],"(Proj.*.muts.maf)")[,1]; batchid = str_replace(maf_file_base_name,".muts.maf","");
  ...
  fo_cmds_T = paste0(fo_dir,'/fo_cmds_T.txt')
    ...
    {  
      ...
      T_sample_bam = gsub(",", "",toString(bam_files[as.numeric(grep(sample,bam_files))]))
      N_sample_bam = gsub(",", "",toString(N_bam_files[as.numeric(grep(pat,N_bam_files))]))
      
      T_sample_maf_out = paste0(fo_dir,'/',sample,'.tumor.per_sample_output_fo.maf')
      N_sample_maf_out = paste0(fo_dir,'/',sample,'.normal.per_sample_output_fo.maf')
